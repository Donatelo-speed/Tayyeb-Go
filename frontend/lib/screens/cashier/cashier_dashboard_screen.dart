...
class _CashierDashboardScreenState extends State<CashierDashboardScreen> {
  ...
  void didChangeDependencies() {
    ...
    _displayName   = appUser?.displayName
        ?? fbUser?.displayName
        ?? 'Cashier';
  }
    ...
    return AppBar(
      ...
      actions: [
        ...
        if (_restaurantId.isNotEmpty)
          _AppBarPendingBadge(
              restaurantId: _restaurantId, locale: locale,
              onTap: () => _select(1)),
        TextButton(
  ...
  Widget _buildBottomBar(LocaleProvider locale) {
    return BottomNavigationBar(
      ...
      elevation: 8,
      items: _navDests.map((d) => BottomNavigationBarItem(
        icon: d.icon == Icons.inbox_outlined
            ? _MobileIncomingIcon(
                restaurantId: _restaurantId, icon: d.icon)
            : Icon(d.icon),
...
class _MobileIncomingIcon extends StatelessWidget {
  final String restaurantId;
  final IconData icon;
  const _MobileIncomingIcon(
      {required this.restaurantId, required this.icon});
...
class _SidebarTodayStat extends StatelessWidget {
  final String restaurantId;
  final LocaleProvider locale;
  const _SidebarTodayStat(
      {required this.restaurantId, required this.locale});
          ...
          StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
            ...
            builder: (ctx, snap) {
              ...
              final revenue = completed.fold<double>(
                0,
                (s, d) =>
                    s + _toDouble(d.data()['totalAmount']),
              );
              final walkIn = docs
                  .where((d) =>
                      d.data()['orderType'] == 'walk_in')
                  .length;
          ...
          if (docs.isEmpty)
            Expanded(
              child: _EmptyPane(
                ...
                bodyAr:
                    'تظهر الطلبات الجديدة هنا فور إرسالها.',
              ),
            )
          else
            Expanded(
              child: ListView.builder(
                ...
                itemBuilder: (ctx, i) => _IncomingOrderCard(
                  doc: docs[i],
                  locale: locale,
                ),
              ),
...
class _IncomingOrderCard extends StatefulWidget {
  ...
  const _IncomingOrderCard(
      {required this.doc, required this.locale});
...
class _IncomingOrderCardState extends State<_IncomingOrderCard> {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        shape: RoundedRectangleBorder(
            borderRadius: BorderRadius.circular(16)),
        title:
            Text(widget.locale.t('Reject Order?', 'رفض الطلب؟')),
        content: Text(widget.locale.t(
  ...
  Widget build(BuildContext context) {
    ...
    final String orderType    =
        data['orderType']    as String? ?? 'delivery';
          ...
          children: [
            ...
            Row(children: [
              ...
              _accepting
                  ...
                  : ElevatedButton.icon(
                      onPressed:
                          _rejecting ? null : _accept,
                      icon: const Icon(
                          Icons.check_rounded, size: 14),
                      label: Text(
...
class _TypePill extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Text(
        isWalkIn
            ? locale.t('Walk-in', 'حضوري')
            : locale.t('Delivery', 'توصيل'),
        style: TextStyle(
          color: isWalkIn ? _kBlue : _kPrimary,
          fontSize: 10,
          fontWeight: FontWeight.w600,
...
class _POSPaneState extends State<_POSPane>
  ...
  Future<void> _checkout(BuildContext ctx) async {
    ...
    try {
      final items = _basket.values.map((e) => {
        'productId':  e.id,
        'name':       e.name,
        'nameAr':     e.nameAr,
        'imageUrl':   e.imageUrl,
    ...
    showModalBottomSheet(
      ...
      builder: (_) => StatefulBuilder(
        builder: (ctx2, setSheet) => _MobileBasketSheet(
          ...
          totalAmount:    _totalAmount,
          locale:         widget.locale,
          onIncrement:    (id) { _increment(id); setSheet(() {}); },
          onDecrement:    (id) { _decrement(id); setSheet(() {}); },
          onClear:        ()  { _clearBasket(); setSheet(() {}); },
          onCheckout:     () async {
            Navigator.pop(ctx2);
            await _checkout(ctx);
          },
      ...
      Padding(
        ...
        child: TextField(
          ...
          decoration: InputDecoration(
            hintText:
                locale.t('Search menu…', 'ابحث في القائمة…'),
            hintStyle:
        ...
        child: Column(children: [
          ...
          Container(
            ...
            decoration: const BoxDecoration(
              color: _kCard,
              border:
                  Border(top: BorderSide(color: _kDivider)),
            ),
            child: Column(children: [
              _TotalRow(
    ...
    return Container(
      ...
      child: SingleChildScrollView(
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          const SizedBox(height: 14),
          Text(l.t('Confirm Order', 'تأكيد الطلب'),
              style: const TextStyle(
                fontSize: 18,
                fontWeight: FontWeight.w800,
                color: _kHead,
          ...
          _TotalRow(l.t('Subtotal', 'الفرعي'),
              widget.subtotal, bold: false),
          const SizedBox(height: 4),
          _TotalRow(l.t('VAT (15%)', 'ضريبة (15%)'),
              widget.taxAmount, bold: false, small: true),
          const Divider(color: _kDivider, height: 14),
          _TotalRow(l.t('Total', 'الإجمالي'),
              widget.totalAmount, bold: true),
          ...
          Row(children: [
            Expanded(
              child: OutlinedButton(
                ...
                child: Text(l.t('Cancel', 'إلغاء'),
                    style:
                        const TextStyle(color: _kSub)),
              ),
            ),
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(locale.isArabic ? titleAr : titleEn,
                style: const TextStyle(
                  fontWeight: FontWeight.w700,
                  fontSize: 15,
                  color: _kHead,
...
class _TotalRow extends StatelessWidget {
  ...
  const _TotalRow(this.label, this.amount,
      {required this.bold, this.small = false});
...
class _EmptyPane extends StatelessWidget {
  ...
  const _EmptyPane({
    required this.icon,
    required this.titleEn, required this.titleAr,
    required this.bodyEn,  required this.bodyAr,
  });
    ...
    return Center(
      child: Padding(
        ...
        child: Column(
          ...
          const SizedBox(height: 10),
          Text(locale.isArabic ? bodyAr : bodyEn,
              textAlign: TextAlign.center,
              style: const TextStyle(
                  fontSize: 13, color: _kSub)),
        ]),
  ...
  Widget build(BuildContext context) {
    ...
    return Center(
      child: Column(
        ...
        const SizedBox(height: 8),
        Text(locale.t('Coming soon', 'قريباً'),
            style: const TextStyle(
