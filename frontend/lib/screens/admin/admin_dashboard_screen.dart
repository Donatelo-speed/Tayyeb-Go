  ...
  void _showKillSwitchDialog(BuildContext context) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
          'This will disable the entire platform. All users will be logged out.\n\nUse only in case of critical system failures.',
        ),
        actions: [
          TextButton(
...
class _LiveStatRowState extends State<_LiveStatRow> {
  ...
  void initState() {
    ...
    _subscriptions = [
      ...
      FirebaseFirestore.instance.collection('restaurants').snapshots().listen((
        s,
      ) {
      ...
      children: [
        ...
        Wrap(
          ...
          children: [
            ...
            _StatCard(
              title: 'Avg Rating',
              value: _orderCount > 0 ? '4.7' : '--',
              icon: Icons.star,
              color: Colors.amber,
            ),
            _StatCard(
              title: 'Commission',
              value: '\$${_commissionTotal.toStringAsFixed(1)}',
              icon: Icons.paid,
              color: Colors.teal,
            ),
          ],
              ...
              try {
                ...
                if (ctx.mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text(
                        'Restaurant "${nameCtrl.text.trim()}" added!',
                      ),
                    ),
                  );
                }
                    ...
                    child: Row(
                      children: [
                            ...
                            children: [
                              Row(
                                children: [
                                  ...
                                  const SizedBox(width: 8),
                                  if (d['cuisine'] != null &&
                                      (d['cuisine'] as String).isNotEmpty)
                                    Container(
                                      padding: const EdgeInsets.symmetric(
                                        horizontal: 6,
                                        vertical: 2,
                        ...
                        Switch(
                          value: isOpen,
                          onChanged: (_) =>
                              _toggleRestaurantStatus(docs[i].id, isOpen),
          ...
          itemBuilder: (context, i) {
            ...
            return Container(
              ...
              child: Row(
                children: [
                  CircleAvatar(
                    backgroundColor: TayyebGoTheme.primaryColor.withValues(
                      alpha: 0.1,
                    ),
                    child: const Icon(
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 2),
                        Text(
                          'Vehicle: ${d['vehicle'] as String? ?? 'N/A'}',
                          style: const TextStyle(
                            color: TayyebGoTheme.textSecondary,
                            fontSize: 12,
                          ),
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<QuerySnapshot>(
      stream: FirebaseFirestore.instance
          ...
          .snapshots(),
      builder: (context, snapshot) {
        final locations = snapshot.hasData
            ? snapshot.data!.docs
              ...
              child: Row(
                ...
                children: [
                  Container(
                    ...
                    child: Text(
                      count > 0
                          ? '$count driver(s) on the map'
                          : 'No driver locations yet',
                      style: TextStyle(
                        color: count > 0
                            ? TayyebGoTheme.successColor
                            : TayyebGoTheme.textMuted,
                  ...
                  children: [
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Text(
                            d['restaurantName'] as String? ?? 'Unknown',
                            style: const TextStyle(fontWeight: FontWeight.bold),
                          ),
                          Text(
                            '\$${((d['amount'] as num?)?.toDouble() ?? 0).toStringAsFixed(2)}',
                            style: const TextStyle(
                              color: TayyebGoTheme.primaryColor,
                            ),
                          ),
    ...
    return ListView(
      ...
      children: [
        ...
        _ReportCard(
          ...
          subtitle: 'Order volume, peak hours, and popular items',
          icon: Icons.shopping_cart,
          color: Colors.green,
        ),
        _ReportCard(
          title: 'User Growth',
          subtitle: 'New user registrations and retention rates',
          icon: Icons.people,
          color: Colors.purple,
        ),
        _ReportCard(
          title: 'Driver Performance',
          subtitle: 'Delivery times, ratings, and completion rates',
          icon: Icons.delivery_dining,
          color: Colors.cyan,
        ),
        _ReportCard(
          title: 'Financial Reports',
          subtitle: 'Commission summaries and payout tracking',
          icon: Icons.attach_money,
          color: Colors.teal,
        ),
      ],
...
class _AdminSettingsViewState extends State<_AdminSettingsView> {
  ...
  void _setNewRegistrations(bool v) =>
      setState(() => _newRegistrationsAllowed = v);
  void _setCommissionRate(double v) => setState(() => _commissionRate = v);
  void _setMaxDrivers(double v) =>
      setState(() => _maxDriversPerZone = v.round());
          ...
          child: Column(
            children: [
              _SettingsSwitch(
                ...
                onChanged: _setAuditLog,
              ),
              const Divider(),
              _SettingsSwitch(
                title: 'Maintenance Mode',
                subtitle: 'Disable all user operations',
                value: _maintenanceMode,
                onChanged: _setMaintenance,
              ),
              const Divider(),
              _SettingsSwitch(
                title: 'Allow New Registrations',
                subtitle: 'Enable or disable sign-ups',
                value: _newRegistrationsAllowed,
                onChanged: _setNewRegistrations,
              ),
            ],
          ),
        ),
        ...
        Container(
          ...
          child: Column(
            ...
            children: [
              ...
              Slider(
                value: _maxDriversPerZone.toDouble(),
                min: 5,
                max: 50,
                divisions: 45,
                label: '$_maxDriversPerZone',
                onChanged: _setMaxDrivers,
              ),
            ],
          ),
        ),
